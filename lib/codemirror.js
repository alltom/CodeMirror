...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var instance = wrapper.CodeMirror = {
      ...
      getScrollInfo: function() {
        return {left: scroller.scrollLeft, top: scrollbar.scrollTop,
    ...
    function clickInGutter(e) {
      var scrollerPos = eltOffset(scroller, true);
      try {
        var localX = e.clientX - scrollerPos.left;
        var localY = e.clientY - scrollerPos.top - paddingTop() + scroller.scrollTop;
      } catch(e) { return false; }
      if (localX >= gutters.offsetWidth) return false;
      if (options.onGutterClick) {
        for (var i = 0; i < options.gutters.length; ++i) {
          var g = gutters.childNodes[i];
          if (g && g.offsetLeft + g.offsetWidth >= localX) {
            if (localY < lineDiv.offsetHeight) {
              var line = lineAtHeight(doc, localY);
    ...
    function needsScrollbar() {
      var realHeight = doc.height + 2 * paddingTop();
    ...
    function visibleLines(scrollTop) {
      ...
      var toHeight = Math.ceil(top + scroller.clientHeight);
    ...
    function updateDisplay(changes, suppressCallback, scrollTop) {
      ...
      var different = from != showingFrom || to != showingTo || lastSizeC != scroller.clientHeight;
      // This is just a bogus formula that detects when the editor is
      // resized or the font size changes.
      if (different) lastSizeC = scroller.clientHeight;
      ...
      if (!intact.length) removeChildren(lineDiv);
        ...
        for (var i = 0; i < intact.length; ++i) {
          ...
          for (var j = cur.from, e = cur.to; j < e; ++j) {
            if (lineNumbers && updateNumbersFrom <= j)
    ...
    function maybeUpdateLineNumberWidth() {
      ...
      if (last.length != currentLineNumberChars) {
        ...
        if (lineGutter) lineGutter.style.width = currentLineNumberWidth + "px";
        else updateGutters();
        return true;
      }
      return false;
    ...
    function updateSelection() {
      ...
      if (posEq(sel.from, sel.to)) { // No selection, single cursor
        var lineObj = getLine(sel.from.line), bidi = bidiInfoAt(lineObj.order, sel.from.ch);
        var measureCh = sel.from.ch, measureSide = "left";
        var otherCh = null, otherSide = "left";
        if (bidi == "ltr") {
        } else if (bidi == "rtl") {
          if (measureCh) measureCh--;
          else measureSide = "right";
        } else if (bidi.after.level == 1) {
          measureCh = bidi.before.to - 1; measureSide = "right";
          if (bidi.after.level < bidi.before.level) otherCh = bidi.before.from;
          else { otherCh = bidi.after.from; otherSide = "right"; }
        } else if (bidi.before.level == 1) {
          measureCh = bidi.after.from;
          if (bidi.after.level < bidi.before.level) otherCh = bidi.before.to - 1;
          else { otherCh = bidi.after.to - 1; otherSide = "right"; }
        }

        var measurePos = headPos = localCoords({line: sel.from.line, ch: measureCh}, true, lineObj);
        cursor.style.left = measurePos[measureSide] + "px";
        cursor.style.top = measurePos.top + "px";
        cursor.style.height = (measurePos.bottom - measurePos.top) * .85 + "px";
        cursor.style.display = "";
        selectionDiv.style.display = "none";

        if (otherCh != null) {
          var otherPos = localCoords({line: sel.from.line, ch: otherCh}, true, lineObj);
          otherCursor.style.display = "";
          otherCursor.style.left = otherPos[otherSide] + "px";
          otherCursor.style.top = otherPos.top + "px";
          otherCursor.style.height = (otherPos.bottom - otherPos.top) * .85 + "px";
        } else { otherCursor.style.display = "none"; }
      } else {
        headPos = localCoords(sel.inverted ? sel.from : sel.to);
        ...
        var drawForLine = function(line, from, toArg) {
          ...
          var coords = function(ch) { return localCoords({line: line, ch: ch}, true, lineObj); };
          iterateBidiSections(lineObj.order, from, toArg == null ? lineLen : toArg, function(from, to, dir) {
        ...
        if (middleFrom <= middleTo) {
          ...
          var botLine = getLine(middleTo),
              bottom = localCoords({line: middleTo, ch: botLine.text.length}, true, botLine);
          ...
          var top = (middleFrom != middleTo || botLine.height > bottom.bottom - bottom.top) ?
            localCoords({line: middleFrom, ch: 0}, true) : bottom;
    ...
    function bidiInfoAt(order, pos) {
      if (!order) return "ltr";
      var before, after;
      for (var i = 0; i < order.length; ++i) {
        var part = order[i];
        if (part.from < pos && part.to > pos) return part.level == 1 ? "rtl" : "ltr";
        else if (part.from == pos) { if (pos) after = part; else return part.level == 1 ? "rtl" : "ltr"; }
        else if (part.to == pos) before = part;
      }
      if (!after) return before.level == 1 ? "rtl" : "ltr";
    ...
    function moveVisually(line, start, dir) {
      ...
      for (var i = 0; i < bidi.length; ++i) {
        var part = bidi[i], ltr = part.level != 1;
        if ((ltr || i == bidi.length - 1 ? part.from <= start : part.from < start) &&
            (ltr || !i ? part.to >= start : part.to > start)) break;
      }
      var target = start + (part.level == 1 ? -1 : 1) * dir;

      while (target != null) {
        if (part.level == 1) {
          if (target == part.from && i < bidi.length - 1) {
            part = bidi[++i];
            target = part && part.from;
          } else if (target == part.to && i) {
            part = bidi[--i];
            target = part && part.to;
          } else break;
        } else {
          if (target < part.from || target > part.to) {
            part = bidi[target < part.from ? --i : ++i];
            target = part && (dir > 0 == part.level == 1 ? part.to - 1 : part.from + 1);
          } else break;
        }
      }
      ...
      function moveOnce(boundToLine) {
        ...
        if (next == null) {
          if (!boundToLine && findNextLine()) {
            var end = dir < 0;
            if (lineObj.order && lineObj.order[0].level == 1) end = !end;
            ch = end ? lineObj.text.length : 0;
          }
          else return false;
        } else ch = next;
        return true;
    ...
    function moveV(dir, unit) {
      ...
      if (unit == "page") {
        var pageSize = Math.min(scroller.clientHeight, window.innerHeight || document.documentElement.clientHeight);
    ...
    function measureLine(line, ch) {
      ...
      var left = anchor.offsetLeft, right = anchor.offsetLeft + anchor.offsetWidth;
      if (ie) {
    ...
    function localCoords(pos, inLineWrap, lineObj) {
      var off = heightAtLine(doc, pos.line) - (inLineWrap ? displayOffset : 0);
      var sp = measureLine(lineObj || getLine(pos.line), pos.ch);
    ...
    function coordsChar(x, y) {
      ...
      var lineObj = getLine(lineNo), textLen = lineObj.text.length;
      if (!textLen) return {line: lineNo, ch: 0};
      ...
      var bidi = lineObj.order;
      if (!bidi) {
        var from = 0, fromX = 0, to = textLen, toX;
        // Guess a suitable upper bound for our search.
        ...
        var left = function(part, r) { return part.level == 1 ? part.to - (r ? 0 : 1) : part.from; };
        var right = function(part, r) { return part.level != 1 ? part.to - (r ? 0 : 1) : part.from; };
        var fromI = 0, toI = bidi.length - 1;
        var from = left(bidi[fromI]), to = right(bidi[toI]), dist = textLen - 1;
        var fromX = getX(from), toX = getX(to);
        if (x > toX) return {line: lineNo, ch: right(bidi[toI], true)};
        if (x < fromX + cw * .5) return {line: lineNo, ch: left(bidi[fromI], true)};
        for (;;) {
          var off = dist >> 1;
          if (!off) {
            var after = x - fromX < toX - x;
          ...
          dist -= off;
          var part = bidi[fromI], inside = Math.abs(right(part) - from), middle, middleI = fromI;
          if (inside >= off) {
            middle = from + (part.level == 1 ? -1 : 1) * off;
          } else {
            off -= inside + 1;
            for (++middleI;; ++middleI) {
              var part = bidi[middleI], sz = part.to - part.from;
              if (sz < off) off -= sz;
              else {
                middle = left(part) + (part.level == 1 ? -1 : 1) * off;
                break;
              }
            }
          }
          var middleX = getX(middle) + (part.level == 1 ? cw : 0);
          if (middleX > x) { to = middle; toX = middleX; toI = middleI; if (wrongLine) toX += 1000; }
          else { from = middle; fromX = middleX; fromI = middleI; }
        }
      }
    }
  ...
  function Line(text, styles, height) {
    ...
    this.text = text;
    this.height = height;
    this.order = bidiOrdering(text);
  }
  Line.inheritMarks = function(text, orig, height) {
    var ln = new Line(text, null, height), mk = orig && orig.marked;
    if (mk) {
  ...
  Line.prototype = {
    ...
    replace: function(from, to_, text) {
      ...
      this.order = bidiOrdering(this.text);
    ...
    append: function(line) {
      ...
      this.text += line.text;
      if (this.order || line.order) this.order = bidiOrdering(this.text);
    ...
    getElement: function(makeTab, wrapAt, compensateForWrapping) {
        ...
        span = function(text, style) {
          ...
          if (wrapAt >= outPos && wrapAt < outPos + l) {
            ...
            if (cut + 1 == l) {
              ...
              pre.anchor = span_(text.slice(cut, cut + 1), style || "");
              if (compensateForWrapping && spanAffectsWrapping.test(text.slice(cut, cut + 2)))
                pre.appendChild(elt("wbr"));
  ...
  var bidiOrdering = (function() {
    ...
    return function charOrdering(str) {
      if (!bidiRE.test(str)) return null;
