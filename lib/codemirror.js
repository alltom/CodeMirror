  ...
  function CodeMirror(place, givenOptions) {
    ...
    function updateSelection() {
      ...
      if (collapsed) {
        ...
        cursor.style.height = (headPos.yBot - headPos.y) * .85 + "px";
        cursor.style.display = "";
        selectionDiv.style.display = "none";
      } else {
        fromPos.y = Math.max(0, fromPos.y); toPos.y = Math.max(0, toPos.y);
        ...
        if (sel.from.ch) {
          ...
          add(fromPos.x, fromPos.y, right, fromPos.yBot - fromPos.y);
        }
        var middleStart = Math.max(0, sel.from.ch ? fromPos.yBot - .5 : fromPos.y);
        var middleHeight = toPos.y + (sel.from.ch ? 1 : 0) - middleStart;
        if (middleHeight > 2)
          add(paddingLeft(), middleStart, 0, middleHeight);
        if ((!sameLine || !sel.from.ch) && sel.to.ch)
          add(paddingLeft(), toPos.y, clientWidth - toPos.x, toPos.yBot - toPos.y);
    ...
    function moveV(dir, unit) {
      ...
      if (unit == "page") {
        ...
        y = pos.y + dir * pageSize;
      } else if (unit == "line") {
        y = dir > 0 ? pos.yBot + 3 : pos.y - 3;
      }
    ...
    function measureLine(line, ch) {
      var pre = line.getElement(makeTab, ch, options.lineWrapping);
      removeChildrenAndAdd(measure, pre);
      var anchor = pre.anchor, left = anchor.offsetLeft;
      ...
      if (ie && top == 0 && left == 0) {
        anchor = anchor.parentNode.insertBefore(elt("span", "x"), anchor.nextSibling);
        top = anchor.offsetTop; left = anchor.offsetLeft;
      }
      anchor.style.verticalAlign = "bottom";
      var bottom = anchor.offsetTop + anchor.offsetHeight;
      // Work around Opera issue -- we can't get reliable wrapping
      // info if we put anything in the anchor, but we can't get a
      // proper height on empty nodes.
      if (!eolSpanContent && bottom == top && ch == line.text.length) {
        setTextContent(anchor, " ");
        bottom = anchor.offsetTop + anchor.offsetHeight * (left == anchor.offsetLeft ? 1 : .5);
      }
      return {top: top, bottom: bottom, left: left};
    }
    function localCoords(pos, inLineWrap) {
      var x, y = heightAtLine(doc, pos.line) - (inLineWrap ? displayOffset : 0);
      var sp = measureLine(getLine(pos.line), pos.ch);
      return {x: sp.left, y: y + sp.top, yBot: y + sp.bottom};
    }
        ...
        span = function(text, style) {
          ...
          if (wrapAt >= outPos && wrapAt < outPos + l) {
            ...
            if (outPos == wrapAt && outPos == len)
              pre.anchor = pre.appendChild(elt("span", eolSpanContent));
          }
        };
      }
